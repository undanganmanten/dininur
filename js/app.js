...
const play = (btn) => {
  ...
  if (!isPlay) {
    ...
    audio.play();
    btn.innerHTML =
      '<i class="fa-solid fa-circle-pause" style="color:#d78c90"></i>';
  } else {
    btn.setAttribute("data-status", "false");
    audio.pause();
    btn.innerHTML =
      '<i class="fa-solid fa-circle-play" style="color:#d78c90"></i>';
  }
};
...
function add_task() {
  ...
  if (
    ...
    input_date.value.length != 0 &&
    input_hadir.value.length != 0
  ) {
    // our boxes have data and we take database
    var key = firebase.database().ref().child("dininur").push().key;
    var task = {
    ...
    var updates = {};
    updates["/dininur/" + key] = task;
    firebase.database().ref().update(updates);
    create_unfinished_task();
    swal(
      "Pesan telah terkirim",
      "Silakan cek pesan anda di kolom yang sudah ada",
      "success"
    );
